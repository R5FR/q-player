...
pub async fn fetch_app_credentials() -> Result<AppCredentials> {
    ...
    for url in &script_urls {
        ...
        if app_id.is_empty() {
            if let Some(id) = scan_app_id(&bundle) {
...
fn extract_script_urls(html: &str) -> Vec<String> {
    ...
    while let Some(link_pos) = html[pos..].find("<link") {
        ...
        if tag_lower.contains("rel=\"preload\"") && tag_lower.contains("as=\"script\"") {
                ...
                if combined.len() > 44 {
                    ...
                    match URL_SAFE.decode(encoded) {
                        Ok(decoded) => match String::from_utf8(decoded) {
                            Ok(secret) => {
                                debug!(tz, secret_len = secret.len(), "Successfully derived secret");
                        ...
                        Err(e) => {
                            trace!(tz, error = %e, "Base64 decode failed");
                        }
                    }
                }
            }
